...
> **참고**: 현재 macOS Apple Silicon (M1/M2/M3)만 지원합니다. Intel Mac 및 Windows/Linux 버전은 추후 제공 예정입니다.
...
위의 다운로드 링크에서 DMG 파일을 받아 설치하세요.

1. DMG 파일 다운로드 및 실행
2. 애플리케이션을 Applications 폴더로 드래그
3. 앱 실행

**⚠️ macOS 보안 경고 해결 방법**

앱을 처음 실행할 때 "악성 소프트웨어가 있는지 확인할 수 없습니다" 경고가 나타날 수 있습니다. 이는 앱에 Apple Developer 서명이 없기 때문입니다.

**해결 방법:**

1. **방법 1**: 우클릭으로 열기
   - Applications 폴더에서 앱을 찾습니다
   - 앱을 **우클릭(또는 Control + 클릭)** → **열기** 선택
   - "열기" 버튼 클릭
   - 이후에는 일반적인 방법으로 실행 가능

2. **방법 2**: 시스템 설정에서 허용
   - 시스템 설정 → 개인정보 보호 및 보안
   - "확인되지 않은 개발자..." 메시지 옆의 **"확인 없이 열기"** 클릭

3. **방법 3**: 터미널에서 Gatekeeper 제거
   ```bash
   xattr -cr /Applications/Claude\ Slack\ Bridge.app
   ```
